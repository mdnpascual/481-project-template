...
Secondly, we would want to implement other seach features so we can accomodate for more knowledgable users that might not needs to input all the ingredients and available equipment one by one. 

Next, we would like to implement a touch-free step-by-step recipe viewer so the user can just put the phone or tablet in one spot and they would just need to glance from time to time on the screen and allow the steps to be moved using boice commands.

Finally before shipping, we would add gamification features like achievments, everyday goals, levels and experience, perks and coupons and social features to keep our younger audiences engaged and interested to the continious use of our cooking instructor.

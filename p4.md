...
Targeting younger audiences means they may not be responsible for groceries. They also will need a lot more flexibility for their time availability. We will need to build tools to allow them to still learn with the restriction of only using the immediate ingredients that are available for their use. We also need to consider what tools and equipment that they have in their kitchen.

### Recipes are still boring to look at

While recipes has made into in the interwebs, it still hasn’t changed a lot compared to cooking books. We want to make cooking engaging and as much fun as possible especially that our target audience would need more stimulation to keep their interest on the learning process.
...
Ingredient matching allows the user to input all their available ingredients in their disposal and let the app do the heavy lifting of matching recipes that the user can do based on the inputs.
...
If the user really wants to make a specific recipe that they don't have all the ingredients to make it, we are implementing a grocery list feature to allows the user to quick add missing ingredients on their selected recipe in their list of ingredients that they need to buy next time.

### Hyperlink Sub-Steps

We are including underlined hyperlinks for vague steps or uncommon ingredients to allow our audience to learn more about it very quickly. This will be either a clarification or a video demonstration for concepts/steps or additional information for uncommon ingredients.
...
For our next iteration, we would focus first on fixing Existing Bugs & critical search options in Axure. This includes correcting heuristic problems, mainly the timer issues and implementing utensil matching and Ingredient Quantity.

Secondly, we would want to implement other search features so we can accommodate for more knowledgeable users that might not needs to input all the ingredients and available equipment one by one. 

Next, we would like to implement a touch-free step-by-step recipe viewer so the user can just put the phone or tablet in one spot and they would just need to glance from time to time on the screen and allow the steps to be moved using voice commands.

Finally before shipping, we would add gamification features like achievements, everyday goals, levels and experience, perks and coupons and social features to keep our younger audiences engaged and interested to the continuous use of our cooking instructor.
